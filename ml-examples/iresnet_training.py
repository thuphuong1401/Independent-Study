...
root_dir = os.path.join("/scratch/pvu3/Indep Study/netcdf/cumulo-tiles/npz")
...
trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=train_sampler, num_workers=4)

print(os.path.join(root_dir, "label"))
print(len(train_dataset))


# in the following, batch_size corresponds to number of swaths (each swath contain multiple tiles)
valloader = torch.utils.data.DataLoader(val_dataset, batch_size=1, collate_fn=tile_collate, shuffle=False, num_workers=4)
testloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, collate_fn=tile_collate, shuffle=False, num_workers=4)
