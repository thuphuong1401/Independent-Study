...
from src.resnet_loader import CumuloDataset
...
t_size = 3
nb_classes = 8
batch_size = 1 # number of tiles per batch 
lr = 0.001
weight_decay = 5e-4

root_dir = os.path.join("/scratch/pvu3/Indep Study/netcdf/fail_dataset/npz")
...
trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=1, sampler=train_sampler, num_workers=1, drop_last = True)


# in the following, batch_size corresponds to number of swaths (each swath contain multiple tiles)
valloader = torch.utils.data.DataLoader(val_dataset, batch_size=1, collate_fn=tile_collate, shuffle=False, num_workers=1, drop_last=True)
testloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, collate_fn=tile_collate, shuffle=False, num_workers=1, drop_last = True)
...
model.conv1 = nn.Conv2d(13, 64, kernel_size=1, stride=1, padding=10, bias=False)

# change the number of classes output channel
model.fc = nn.Linear(512, 8)

#model.convNormActivation = nn.Conv2d(13, 64, 1, 1, 3, bias=False)
...
plt.savefig("ResNet-small-confusion-matrices.png", bboxes="tight")
